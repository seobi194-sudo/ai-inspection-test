    ...
    stages: [
      {
        ...
        expected: "단열재 시공 전 벽면 전체와 주요 접합부가 보이고, 시공을 방해하는 큰 잔재나 젖은 부위가 있는지 확인할 수 있어야 한다.",
        criteria: "이 단계는 외관의 깨끗함이 아니라 단열재 시공을 방해하는 요소가 사진에 명확히 보이는지 확인한다. 콘크리트의 색 차이, 철거·그라인딩 흔적, 접착제나 기존 마감 자국, 먹줄·표시, 검은 점 또는 마른 얼룩만으로 곰팡이·누수나 기준미달을 단정하지 않는다. 사진만 보고 곰팡이를 진단하지 않는다. 큰 돌출물, 들뜨거나 떨어질 수 있는 잔재, 다량의 파편, 큰 미보수 균열·구멍, 고인 물 또는 젖어 흐르는 흔적이 명확할 때만 기준미달로 판단한다. 얼룩은 있으나 면이 건조해 보이고 단열 시공을 막는 요소가 보이지 않으면 사진 기준 적합으로 판단한다.",
        items: ["시공면 전체 확인", "큰 돌출·잔재 여부", "명확한 습윤·박리 여부"]
      },
      {
        id: "i2",
        label: "단열재 전체 시공상태",
...
13. 바탕면 검수에서는 검은 점, 색 차이 또는 마른 얼룩만으로 곰팡이·누수라고 진단하거나 기준미달로 표시하지 않습니다. 실제 습윤, 물 흐름, 박리 또는 시공 방해요소가 사진에 명확히 보여야 기준미달입니다.
14. 발주자와 시공자 모두 이해할 수 있는 중립적인 한국어로 답합니다.`;
}
